...
# clarify things. His numeral system has base 26, and its digits are represented by English capital letters - A for 0,
# B for 1, and so on.
#
...
# [output] array.string
#
# An array of strings in the format "letter1 + letter2", where "letter1" and "letter2" are correct one-digit numbers
# in the new numeral system. The strings should be sorted by "letter1".
#
# Note that "letter1 + letter2" and "letter2 + letter1" are equal pairs and we don't consider them to be different.


def solution(number):
    num_system = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    n = num_system.find(number)
    l = []
